...
def options():
    ...
    parser.add_argument('-f', '--infiles', type=str, required=True,
                        help='comma-separated list of input file names')
    parser.add_argument('-c', '--count', type=int, default=10,
                        help='number of iterations (default: 10)')
    parser.add_argument('-r', '--read_count', type=int, default=10000,
    ...
    parser.add_argument('-e', '--context', type=str, help='context')
...
def make_emission_probs(acc_in, inacc_in):
    ...
    print(len(emission_probs[0]))
    print(len(emission_probs[1]))
...
def generate_training(indir, infiles, count, read_count, random_seed, context):
    ...
    for f in infiles:
        ...
        for chrom in train_reads['chrom'].unique():
            if chrom != 'chrM':
                tmp = pd.read_parquet(
                    '/gscratch/stergachislab/bmallo/large_home/hmm-footprint-caller/pEF_GFP_Gus/Infiles/parquet/methyl_filtered_50.m6a/methyl_filtered_50.m6a.pq', columns=train_reads.loc[train_reads['chrom'] == chrom].index)
                # indir_f+f+'_'+chrom+'.pq', columns=train_reads.loc[train_reads['chrom'] == chrom].index)
...
def trained_HMM(emission_probs, train_arrays):
    ...
    for i in list(train_arrays.keys()):
        ...
        model = hmm.MultinomialHMM(
            n_components=n_states, init_params='', params='st', n_iter=1000)
...
emission_probs = np.array(make_emission_probs(
    inref+'accessible_probs.tsv', inref+'inaccessible_probs.tsv'))
train_arrays, train_rids = generate_training(
