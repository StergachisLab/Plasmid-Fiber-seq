...
def encode_me(rid, read, read_info, context):
    ...
    no_me=np.arange(start, end)
    no_me=np.delete(no_me, me)
    hexamers=pd.read_hdf(context, key='chrM', start=start, stop=end)
       ...
       # me_encode=pd.read_hdf(context, key=chrom).to_numpy().T[0]
        ...
        for rid, read in reads.iterrows():
            ...
            if len(read_encode)>0:
                read_encode=np.append(read_encode,np.append(read_encode,read_encode))
              # print(read_encode.shape)
                ...
                if len(combined)>0:
                    ...
                    combined=np.zeros(16569)+16569

                combined=combined[16569:16569*2]
                ...
                if len(combined)<16569:
                    combined=[-16569]*16569 
                fp_dic[rid]=combined
